...
public class Activity {
	...
	public void nextStateA(StateMachine SM){
		...
		if(DNSWrapper.isQuery(payload)){
			...
			try {
				ResultSet rs = ProtocolStateMachine.dbConnect.getConnection().prepareStatement("SELECT TRANSACTION_ID FROM "+ProtocolStateMachine.dbConnect.getDBName()+".Query "+"WHERE TRANSACTION_ID="+trID).executeQuery();
				if(!rs.first())
			...
			try {
				ResultSet rs = ProtocolStateMachine.dbConnect.getConnection().prepareStatement("SELECT TIME_STAMP FROM "+ProtocolStateMachine.dbConnect.getDBName()+".Query "+"WHERE TRANSACTION_ID="+trID).executeQuery(); //ProtocolStateMachine.dbConnect.retrieveResultSet("SELECT TIME_STAMP FROM "+ProtocolStateMachine.dbConnect.getDBName()+".Query "+"WHERE TRANSACTION_ID="+trID);
				if(rs.first()){
	...
	public void nextStateE(StateMachine SM){
		...
		if(DNSWrapper.isResponse(payload)){
			...
			try {
				rs2 =  ProtocolStateMachine.dbConnect.getConnection().prepareStatement("SELECT TIME_STAMP FROM "+ProtocolStateMachine.dbConnect.getDBName()+".Referral "+"WHERE TRANSACTION_ID="+trID+" ORDER BY TIME_STAMP DESC").executeQuery();//ProtocolStateMachine.dbConnect.retrieveResultSet("SELECT TIME_STAMP FROM "+ProtocolStateMachine.dbConnect.getDBName()+".Referral "+"WHERE TRANSACTION_ID="+trID+" ORDER BY TIME_STAMP DESC");
				if(rs2.first()){
